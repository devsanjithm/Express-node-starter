 ...
 */
const saveToken = async (token, userId, expires, type, is_active = false) => {
  const tokenDoc = await prismaInstance.tokens.create({
    data: {
      token,
...
const verifyToken = async (token, type) => {
  ...
  const tokenDoc = await prismaInstance.tokens.findFirst({ token, type, user: payload.sub, is_active: false });
  if (!tokenDoc) {
